...
namespace drawing::image::formats
{
    ...
    // 做 8 + 12 个字节的测试
    bool is_png_data(const byte_t * pBuffer, size_t length)
    {
        if (length < 21)
            return false;
    ...
    core::error png_create(image_codec_context & ictx, const byte_t * buffer, size_t length, image_data_t & image)
    {
        png_source buff;
        buff.buffer = buffer;
        buff.length = length;
