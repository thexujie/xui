...
namespace drawing
{
	...
	public:
		...
		virtual core::error update(std::u8string_view text, const drawing::font & font, core::color color) = 0;
		virtual core::sizef bounds() const = 0;

		virtual size_t clusterCount() const = 0;
		virtual const cluster & clusterAt(size_t cindex) const = 0;
		virtual const cluster & findCluster(size_t tindex) const = 0;
		virtual const cluster & findCluster(float32_t pos) const = 0;
		
		virtual std::tuple<size_t, core::rectf> textRect(size_t toffset, size_t tlength) const = 0;
	};
}
