...
namespace agg
{
    ...
    void path_base<VC>::invert_polygon(unsigned start)
    {
        ...
        while (start < m_vertices.total_vertices() &&
            !is_vertex(m_vertices.command(start)))
            ++start;
        ...
        while (end < m_vertices.total_vertices() &&
            !is_next_poly(m_vertices.command(end)))
            ++end;
    ...
    public:
        ...
        unsigned last_command() const
        {
            return m_vertices.size() ? m_vertices[m_vertices.size() - 1].cmd : path_cmd_stop;
        }
