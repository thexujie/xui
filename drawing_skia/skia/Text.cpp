...
namespace skia
{
    ...
    std::shared_ptr<class SkTextBlobBuilder> Shaper::builder()
    {
        if (!_builder)
    ...
    uint16_t Shaper::indexFont(const drawing::font & font)
    {
        ...
        {
            auto sktypeface = std::shared_ptr<SkTypeface>(SkTypeface::MakeFromName(reinterpret_cast<const char *>(font.family.c_str()), skia::from(font.style)).release(), skia::skia_unref<>);
            auto hbfont = create_hb_font(sktypeface.get());
            if(!sktypeface || !hbfont)
                throw core::e_not_supported;

            _fonts.push_back({ font, sktypeface, hbfont, skia::fontmetrics(font) });
    ...
    void Text::setFont(span32 range, const drawing::font & font)
    {
    ...
    void Text::setColor(span32 range, uint32_t color)
    {
    ...
    core::error Text::itermize(const drawing::font & font_default, core::color color_default)
    {
        ...
        while (utf16_pos < u16str.length())
        {
            ...
            else
            {
                ...
                auto tf = fontMgr->matchFamilyStyleCharacter(nullptr, skia::from(font_cache.font.style), nullptr, 0, cp_utf8);
                if (tf)
                {
                    drawing::font font_fb = skia::to(*tf, font_cache.font.size);
                    drawing::fontmetrics fmetrics_fb = skia::fontmetrics(font_fb);
                    drawing::fontmetrics fmetrics = skia::fontmetrics(font_cache.font);
                    if(fmetrics_fb.ascent - fmetrics.ascent > 0.49f || fmetrics_fb.descent - fmetrics.descent > 0.49f)
                    {
                        ...
                        //    rate = rate0;
                        //float32_t rate = fmetrics.height / fmetrics_fb.height;
                        font_fb = skia::to(*tf, font_cache.font.size * rate);
                    }
    ...
    std::shared_ptr<SkTextBlob> Text::build(float32_t width)
    {
        ...
        for (size_t sindex = 0; sindex < _segments.size(); ++sindex)
        {
            ...
            if(width > 0 && offset_x + seg.advance.cx + _elid.advance.cx * 3 >= width)
            {
                ...
                if(nclusters > 0)
                {
                    ...
                    SkTextBlobBuilder::RunBuffer runBuffer = builder->allocRunPos(font, nglyphs, nullptr);
...

                {
                    ...
                    SkTextBlobBuilder::RunBuffer runBuffer = builder->allocRunPos(font, 3, nullptr);
            ...
            {
                SkFont font(sk_ref_sp(font_cache.sktypeface.get()), font_cache.font.size);
                SkTextBlobBuilder::RunBuffer runBuffer = builder->allocRunPos(font, seg.grange.length, nullptr);
        ...
        return { builder->make().release(), skia::skia_unref<SkTextBlob> };
    }

    const cluster & Text::findCluster(size_t tindex) const
    {
    ...
    const cluster & Text::findCluster(float32_t pos) const
    {
    ...
    std::tuple<size_t, core::rectf> Text::textRect(size_t toffset, size_t tlength) const
    {
        if(!tlength)
            return { core::npos, {} };
    ...
    core::error TextWraper::layout(float32_t width, drawing::wrap_mode mode)
    {
        _lines.clear();
        _segments.clear();
        ...
        for (size_t cindex = 0, iindex = 0; cindex < _clusters.size(); ++cindex)
        {
            ...
            if (core::equal(width, 0.0f) || mode == drawing::wrap_mode::none)
                ;
