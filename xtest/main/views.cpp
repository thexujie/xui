...
{
public:
    UserViewItem(const std::u8string & text, drawing::wrap_mode wrap_mode = drawing::wrap_mode::none) : _text(text), _wrap_mode(wrap_mode) {}
...
void views_main()
{
	...
	}
	{
		...
		tln->setFont({ u8"", drawing::font().size * 4 });
		tln->setText(u8"Hello Hello");
    ...
    }
    {
        ...
        auto tln = std::make_shared<ui::controls::TextLine>();
        tln->setFont(drawing::font(u8"", drawing::default_font().size * 2));
        tln->setText(rtft);
    ...
    }
	{
		std::u8string rtft = u8"hello لغة عربية‎𪚥𪚥བོད་ཀྱི་སྐད་ཡིག།𪚥ยิ้ยิ้ยิ้😂🌐🍪🍕🚀ยิ้ยิ้ยิ้㌶㌫ ق قق ققق قققق";
		auto tln = std::make_shared<ui::controls::TextLine>();
		tln->setFont(drawing::font(u8"", drawing::default_font().size * 1.5f));
		tln->setText(rtft);
		tln->setSize({ 100_per, core::auto_value });
		tln->setMargin({ 0.2_em });
