...
namespace ui
{
	...
	std::u8string ListView::styleName() const
	{
		if(_actived)
			return u8"listview:active";
		else if(_hovered)
			return u8"listview:hover";
		else
			return u8"listview";
	}
    ...
    {
		auto item_details = std::make_shared<ui::MenuItem>(drawing::Image(u8"icon.png"), u8"œÍœ∏–≈œ¢", ui::shortcut(), 0x100, item_flag::none);
		auto item_tile = std::make_shared<ui::MenuItem>(drawing::Image(u8"icon.png"), u8"∆Ω∆Ã", ui::shortcut(), 0x101, item_flag::none);
